...
//	@Delete("DELETE FROM akd_documents WHERE doc_id=#{docID}")
	...
	@Select("SELECT * from akd_documents WHERE cat_id=#{catID} AND status=1 LIMIT #{pagination.limit} OFFSET #{pagination.offset}")
	@Results({
		@Result(property="docID", column="doc_id"),
		@Result(property="title", column="title"),
		@Result(property="des", column="des"),
	...
	ArrayList<Document> getDocumentByCatID(@Param("catID") String catID,@Param("pagination") Paging pagination);
	
	
	
	@Select("SELECT * from akd_documents WHERE cat_id=#{catID} AND user_id=#{userID} AND status=1 LIMIT #{pagination.limit} OFFSET #{pagination.offset}")
	@Results({
		@Result(property="docID", column="doc_id"),
		@Result(property="title", column="title"),
		@Result(property="des", column="des"),
	...
	ArrayList<Document> getDocumentByCatIDAndUserID(@Param("catID") String catID,@Param("userID") int userID,@Param("pagination") Paging pagination);
	...
	ArrayList<Document> getDocumentByPopularAndPagination(@Param("pagination") Paging pagination);
	...
	@Select("SELECT DISTINCT doc.doc_id,doc.title,doc.doc_type_num,doc.des,doc.embed_link,doc.thumbnail_url,doc.export_link,doc.view,doc.share,doc.user_id, doc.status, doc.created_date, doc.cat_id FROM akd_documents doc INNER JOIN akd_logs lg ON doc.doc_id=lg.doc_id WHERE doc.cat_id IN(SELECT dd.cat_id FROM akd_documents dd INNER JOIN akd_logs ll ON dd.doc_id=ll.doc_id WHERE ll.user_id=#{userID} AND dd.status=1 ORDER BY ll.date DESC) LIMIT #{pagination.limit} OFFSET #{pagination.offset}")
	@Results({
		@Result(property="docID", column="doc_id"),
		@Result(property="title", column="title"),
		@Result(property="des", column="des"),
	...
	ArrayList<Document> getDocumentByRecommendedAndPagination(@Param("userID") int userID, @Param("pagination") Paging pagination);
	...
	@Select("SELECT * FROM akd_documents ORDER BY to_timestamp(created_date, 'dd-MM-yyyy HH24:MI:SS') DESC LIMIT #{pagination.limit} OFFSET #{pagination.offset}")
	...
	ArrayList<Document> getDocumentByNewPostAndPagination(@Param("pagination") Paging pagination);
